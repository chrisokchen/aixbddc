1. FOR EACH 步驟 8 之 state `target_path`：TRIGGER `upsert`，`path` 為 `data/<target_path>`（相對 `${TRUTH_BOUNDARY_ROOT}`）；`change_type` 依下列規則選一個：
    1. 檔已存在且 plan 確定改寫 → `update`
    2. 新 target_path → `add`
    3. 仍待 sourcing 決策才能落地 → `conditional_update`
    `impact_summary` 用現在式一句話描述本 skill 對該 state schema 檔的規格增量。
    ```bash
