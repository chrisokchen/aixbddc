...
- 業務語言: 人物名字（顧客 Alice）、顧客或客服實際引用的訂單編號（電商格式 2406KX8Q7M2P9T，英數碼即字串、永遠加引號）
- 技術氣味: 顧客看不到的純內部 ID（自增 order-001、user-001、uuid）
- 判別關鍵在顧客或客服是否實際引用而不是長得像不像代碼：真實訂單編號雖為英數碼，但顧客拿它報單、客服拿它查單，屬業務語言；自增序號與 uuid 顧客看不到，屬技術氣味。
...
- 識別: 人物名與業務序號語意為 Alice／2406KX8Q7M2P9T，字面加 `"..."`
- 狀態: 中文「處理中」，字面加 `"..."`
